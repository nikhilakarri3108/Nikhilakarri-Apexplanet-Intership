...
Key features: Packet list: Shows all captured packets in real time. Filters: You can use both capture filters (to limit what is recorded) and display filters (to focus on what you see after capturing).
...
Key commands: nmap [target] - Basic scan of a host. nmap -sS - TCP SYN Scan (Stealth scan). nmap -sV - Service Version detection. nmap -p [port] - Scans a specific port or port range.
...
Key options: -v: Verbose mode (gives more details). -z: Zero-I/O mode (used for scanning only, without sending data). -l: Listen mode (waits for an incoming connection). -p: Specifies the source port.
